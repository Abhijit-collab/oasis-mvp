...
const floorLevel = (name) => floorLevelFromName(name);
...
export function buildFilterPanelUnits(units = UNITS) {
  ...
  return Object.values(units).map((u) => ({
    ...
    block: (u.block && String(u.block).replace(/^Block\s+/i, "")) || blockByUnit[u.id] || "A",
    facing: u.facing ? facingCode(u.facing) : undefined,
